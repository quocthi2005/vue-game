<template>
  <div class="d-flex justify-content-center">
    <div v-if="showScore">
      <b-card title="Results" style="max-width: 20rem;" class="mb-2">
        <b-card-text>You Scored {{ score }} of {{ questions.length }}</b-card-text>
        <b-button href="/">Start Agian</b-button>
      </b-card>
    </div>
    <div class="card-q" v-else>
      <span v-if="!startQuiz">
        <b-card
          img-src="https://cf-cdn.relax-gaming.com/uploads/images/history-e732d170834d12f105b60eb88fe220c5.jpg"
          img-alt="Image" img-top title="Simple Quiz Application" style="max-width: 20rem;" class="mb-2">
          <b-button variant="success" @click="startQuizFunc()">Start Quiz</b-button>
        </b-card>
      </span>
      <span v-else>
        <b-card title="Simple Quiz Application" style="max-width: 20rem;" class="mb-2">
          ...
          <div class="answer-section">
            <b-button :key="index" v-for="(option, index) in questions[currentQuestion].answerOptions"
              @click="handleAnswerClick(option.isCorrect)" class="ans-option-btn" variant="primary">
